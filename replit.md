...
- July 09, 2025. Fixed user export functionality that was returning HTML instead of CSV data. The issue was authentication-related - when users weren't properly logged in, the export request would receive 401 errors and redirect to login page (HTML) instead of CSV. Added proper authentication handling in useCsvExport hook with 401 error detection, user feedback, and automatic login redirect. Backend API was working correctly but frontend session management needed improvement. Export now works properly when users are authenticated.
- July 09, 2025. Resolved device vs user export inconsistency by standardizing both to use window.open() method. Device export worked while user export failed due to complex iframe handling. Simplified useCsvExport hook to use consistent approach across all export functions. Both device and user exports now work reliably with proper session cookie handling.
- July 09, 2025. Fixed user export authentication issue by switching from window.open() to form submission method. The root cause was that window.open() creates new browser context without session cookies, while form submission maintains the existing session. Updated useCsvExport hook to use hidden form submission which preserves session cookies properly. Both device and user exports now work consistently with proper authentication.
- July 09, 2025. Performed comprehensive database cleanup as requested. Cleared all devices (132), categories (18), sites (4), activity logs (546), assignment history (148), QR codes (4), maintenance records (1), and software assignments (14). Database now contains only users and system configuration, ready for fresh data import.
- July 09, 2025. Performed additional database cleanup. Cleared all software (2), notifications (3), and departments (10). Updated 46 users to remove department assignments. Database now contains only users and core system configuration.
- July 09, 2025. Fixed device import functionality to create exact matches from CSV data. Removed name transformations and mappings - now creates categories and sites with exact names from CSV file (e.g., "BRD" creates site "BRD", "AV" creates category "AV"). Cleared database completely for fresh import. Import now processes CSV data exactly as provided without any name modifications.
