...
// Test password reset email template (admin only)
router.post('/settings/email/test-password-reset', isAuthenticated, isAdmin, async (req: Request, res: Response) => {
  try {
    // Get the current email settings
    const emailSettings = await storage.getEmailSettings();
    ...
    const result = await updatedMailgunService.sendPasswordResetEmail(currentUser.email, testPassword, userName);
    
    if (result.success) {
      console.log(`✓ Test password reset email sent successfully to: ${currentUser.email}`);
    } else {
      console.log(`✗ Test password reset email failed: ${result.message}`);
    }
    
    return res.json(result);
  } catch (error) {
    console.error('Error with password reset email test:', error);
    return res.status(500).json({ 
      success: false, 
...
// Test welcome email template (admin only)
router.post('/settings/email/test-welcome', isAuthenticated, isAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const testPassword = 'WelcomePass123!';
    const userName = `${currentUser.firstName} ${currentUser.lastName}`;
    const result = await updatedMailgunService.sendWelcomeEmail(currentUser.email, testPassword, userName);
    
    if (result.success) {
      console.log(`✓ Test welcome email sent successfully to: ${currentUser.email}`);
    } else {
      console.log(`✗ Test welcome email failed: ${result.message}`);
    }
    
    return res.json(result);
  } catch (error) {
    console.error('Error with welcome email test:', error);
    return res.status(500).json({ 
      success: false, 
